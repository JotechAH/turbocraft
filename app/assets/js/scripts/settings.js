    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    'Warning<br>',
                    'Tu dois êtres connecté à un compte afin de jouer, tu seras ramené sur la page de connection, es-tu sûr de  vouloir continuer?',
                    'Oui',
                    'Annuler'
                )
                setOverlayHandler(() => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle mise à jour dispo`
        ...
        settingsUpdateTitle.innerHTML = "T'as la dernière version"
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus("Check s'il y a une update", false, () => {
